...
const { getId } = require('../utils/claudinaryFunction');
...
async function followPlaylist(req, res, next) {
  try {
    const _id = req.headers._id;
    const user = await UserAction.findOne({ userId: _id });
    if (user.error) {
      return res
        .status(400)
...
async function getAllPlaylists(req, res, next) {
  try {
    const _id = req.headers._id;
    const user = await UserAction.findOne({ _id });
    if (user.error) {
      return res
        .status(400)
        .send({ error: 'The user has not been found, please try again' });
...
async function addTrack(req, res, next) {
  ...
  try {
    const findPlaylist = await PlaylistAction.findOne({ _id: playListId });

    const numSongs = findPlaylist.data.numberSongs;

    const addedTrack = await PlaylistAction.findByIdAndUpdate(
      playListId,
      {
...
async function orderTracks(req, res, next) {
  ...
  const { id } = req.params;
  try {
    const playlist = await PlaylistAction.findOne({ _id: id });
    ...
    const updatedPlaylist = await PlaylistAction.findByIdAndUpdate(
      { _id: id },
      { tracks: orderedList },
      { new: true }
    );
...
//!Revisar todas las acciones de los controladores.
module.exports = {
  // createPlaylist,
  // followPlaylist,
  // getAllPlaylists,
